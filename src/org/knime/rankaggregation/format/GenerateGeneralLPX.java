...
public class GenerateGeneralLPX extends AbstractGenerateILPFile {
	...
	public GenerateGeneralLPX(BufferedDataTable tableSpecInput, Path localPath, URL url, ExecutionContext exec) {
	...
	public void generate() throws IOException, CanceledExecutionException {
		...
		try (BufferedWriter writer = openWriter(localPath, url)) {
			...
			writer.write("\n");
			writer.write("// First Constraints");
			writer.write("\n");
			for (int i = 0; i < variables.size(); i = i + 2) {
				String Xba = variables.get(i);
				String Xab = variables.get(i + 1);
				String condicion = "row" + rowId + ": " + Xab + " + " + Xba + " = 1;";
				writer.write(condicion);
				writer.write("\n");
				rowId++;
				condicion = "row" + rowId + ": " + Xba + " + " + Xab + " = 1;";
				writer.write(condicion);
				writer.write("\n");
				rowId++;
			}
			...
			for (int i = 0; i < variables.size(); i++) {
				...
				for (int j = 1; j <= numColumns; j++) {
					...
					if (varIzq.compareTo(varCentro) != 0 && j != noFin && ultimo != j) {
						...
						String condicion = "row" + rowId + ": " + varIzq + " + " + varCentro + " + " + varDer
								+ " >= 1;";
						writer.write(condicion);
						writer.write("\n");
						rowId++;
